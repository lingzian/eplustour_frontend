...
var ITEM_HEIGHT = (function(){
  var dpr = 1;
  var isIPhone = window.navigator.appVersion.match(/iphone/gi) || window.navigator.appVersion.match(/iPad/gi);
  if(window.devicePixelRatio && isIPhone){
    dpr = Math.floor(window.devicePixelRatio);
  }
  return Math.ceil(window.screen.width * 0.1) * dpr;
})();
